...
AudioFFT::AudioFFT(std::shared_ptr<AudioBuffer> audioBuffer,
  ...
  _output =
      (fftw_complex *)fftw_malloc(sizeof(fftw_complex) * (_fftSize / 2 + 1));
  _plan = fftw_plan_dft_r2c_1d(_fftSize, _input, _output, FFTW_MEASURE);
  // ^ using a plan (from fftw) here makes the fft much faster to run over and
  // over again
}
...
int AudioFFT::performFFT(std::shared_ptr<uint32_t[]> out, size_t buckets) {
  ...
  int16_t *samples = reinterpret_cast<int16_t *>(
      bufferData); // might be able to consolidate these casts to one static
                   // cast to double?
  ...
  // ratio is so each bucket spans a constant factor of the frequncy ; f_min *
  // r^num_buckets = f_max
  ...
  for (size_t b = 0; b < buckets; ++b) {
    ...
    // clamp to valid range [1 ... N/2] (not sure how fast this call is, might
    // change)
    ...
    // NEW: find *max* magnitude in bin before dB conversion (rather than prior
    // *sum*, as buckets now unequal width)
    double maxMag = 0.0;
    ...
    // the db conversion may be wholly unnecessary I have not made up my mind
    // but its simple enough to do so I'm doing it for now
    ...
    out[b] = static_cast<uint32_t>(
        dBNormalized); // store scaled dB as uint32_t[] for HJ's portion
  }
