...
void Service::_doService() {
  ...
  int counter = 0;
  while (_running) {
    auto acquired = _releaseService.try_acquire_for(std::chrono::milliseconds(
        _period *
        2)); // TODO: wait for less time, so that we can check if service is
             // still running. Add a helper method for this.
    ...
    if (!acquired) {
      counter++;
      _logger->log(logger::ERROR, "Service " + _serviceName +
                                      " did not release in its 2*period: " +
                                      std::to_string(_period * 2) + "ms");

      if (counter > 100) {
        _logger->log(logger::ERROR,
                     "Service " + _serviceName +
                         " has not released in 100 periods. Stopping service.");
...
void Service::release() {
  if (!_serviceStarted.load()) {
    ...
    auto time_released = std::chrono::duration_cast<std::chrono::microseconds>(
                             now - _firstRelease.load())
                             .count();
    auto expected =
        std::chrono::microseconds(_releaseNumber * _period * 1000).count();
...
Sequencer::Sequencer(uint16_t period, uint8_t priority, uint8_t affinity)
    : _period(period), _stats(StatTracker(1000)) {
  setCurrentThreadAffinity(affinity);
  setCurrentThreadPriority(priority);
}

void printServiceStatistics(std::ofstream &file,
  ...
  file << "Execution Time Average: " << executionStats.GetAverageDurationMs()
       << "ms\n";
  file << "Execution Time Max: " << executionStats.GetMaxVal() << "ms\n";
  file << "Execution Time Min: " << executionStats.GetMinVal() << "ms\n";
  file << "Release Time Average Error: " << releaseStats.GetAverageDurationMs()
       << "ms\n";
  file << "Executions that met deadline: "
       << executionStats.GetNumberCompletedOnTime(service->getPeriod()) << "/"
       << executionStats.GetNumElements() << "\n";
  file << "Executions that completed successfully: "
       << service->getStatusCounter()->GetCount(SUCCESS) << "/"
       << executionStats.GetNumElements() << "\n";
  file << "================================================================\n";
}
...
void printSequencerStatistics(std::ofstream &file, StatTracker &stats) {
  ...
  file << "Execution Time Error Average: " << stats.GetAverageDurationMs()
       << "ms\n";
...
void printStatistics(StatTracker &sequencerStats,
...
void Sequencer::_checkPeriodCompatability(uint16_t servicePeriod) {
  if (servicePeriod % _period != 0) {
    throw std::invalid_argument(
        "Service not compatible with sequencer. The service period must be "
        "divisible by the sequencer period.");
  }
}
...
public:
  ISRSequencer(uint8_t period, uint8_t priority, uint8_t affinity)
      : Sequencer(period, priority, affinity), _releaseSequencer(0) {}

  ~ISRSequencer() { deleteTimer(); }

protected:
  void _waitForRelease() override {
    auto acquired = _releaseSequencer.try_acquire_for(
        std::chrono::milliseconds(_period * 2));

    if (!acquired) {
      std::cerr << "Fatal error - sequencer did not release in twice its period"
                << std::endl;
      exit(1);
    }
  }

  void _initializeSequencer() override { setPeriodAlarm(_period); }
...
private:
  ...
  void onAlarm() { _releaseSequencer.release(); }
  ...
  void setPeriodAlarm(long intervalMs) {
    ...
    its.it_interval.tv_nsec =
        intervalMs * 1000000; // Convert milliseconds to nanoseconds
...
public:
  SleepSequencer(uint8_t period, uint8_t priority, uint8_t affinity)
      : Sequencer(period, priority, affinity) {}
...
/////////////// SEQUENCER FACTORY ///////////////////
Sequencer *SequencerFactory::createISRSequencer(uint16_t period,
                                                uint8_t priority,
                                                uint8_t affinity) {
  return new ISRSequencer(period, priority, affinity);
}

Sequencer *SequencerFactory::createSleepSequencer(uint16_t period,
                                                  uint8_t priority,
                                                  uint8_t affinity) {
