...
public:
  MicrophoneService(std::string id, uint16_t period, uint8_t priority,
                    uint8_t affinity,
  ...
  ~MicrophoneService() { delete _logger; }
...
protected:
  ServiceStatus _serviceFunction() override {
    ...
    if (err == -2) {
      ...
      _logger->log(logger::TRACE,
                   "Got " + std::to_string(err) + " frames from microphone");
    }

    bool acquired =
        _fftDone.try_acquire_for(std::chrono::milliseconds(_period));
    if (!acquired) {
      _logger->log(
          logger::ERROR,
          "MicrophoneService timed out waiting for FFT service to finish");
      return FAILURE;
    }
...
public:
  FFTService(std::string id, uint16_t period, uint8_t priority,
             uint8_t affinity,
             std::shared_ptr<logger::LoggerFactory> loggerFactory,
             std::shared_ptr<AudioBuffer> audioBuffer,
             ServiceConfig serviceConfig)
...
protected:
  ServiceStatus _serviceFunction() override {
    ...
    bool acquired =
        _fftReady.try_acquire_for(std::chrono::milliseconds(_period));

    if (!acquired) {
      _logger->log(logger::ERROR,
                   "FFTService timed out waiting for microphone data");
      return FAILURE;
    }
...
public:
  BeeperService(std::string id, uint16_t period, uint8_t priority,
                uint8_t affinity,
                std::shared_ptr<logger::LoggerFactory> loggerFactory,
                std::shared_ptr<AudioBuffer> audioBuffer,
                ServiceConfig serviceConfig)
    ...
    _serviceConfig = serviceConfig;
    _internalBuffer =
        new double[sizeof(double) * _serviceConfig.numberOfBuckets];
    _logger = loggerFactory->createLogger("BeeperService");
  }

  ~BeeperService() { delete[] _internalBuffer; }
...
public:
  LEDBlinker(std::string id, uint16_t period, uint8_t priority,
             uint8_t affinity,
             std::shared_ptr<logger::LoggerFactory> loggerFactory,
             ServiceConfig serviceConfig)
      : Service("ledblinker[" + id + "]", period, priority, affinity,
                loggerFactory) {
    _serviceConfig = serviceConfig;
    _internalBuffer = std::make_shared<uint32_t[]>(
        sizeof(uint32_t) * _serviceConfig.numberOfBuckets);
...
public:
  LogsToFileService(std::string id, uint16_t period, uint8_t priority,
                    uint8_t affinity,
                    std::shared_ptr<logger::LoggerFactory> loggerFactory,
                    ServiceConfig serviceConfig)
      : Service("logstofile[" + id + "]", period, priority, affinity,
                loggerFactory) {
...
void runSequencer(std::shared_ptr<RealTimeSettings> realTimeSettings) {
  std::shared_ptr<logger::LoggerFactory> loggerFactory =
      realTimeSettings->getLoggerFactory();

  int maxPriority = sched_get_priority_max(SCHED_FIFO);
  int minPriority = sched_get_priority_min(SCHED_FIFO);

  Sequencer *sequencer =
      realTimeSettings->createSequencer(10, maxPriority, SEQUENCER_CORE);

  ServiceConfig serviceConfig;
  serviceConfig.numberOfBuckets = 8;

  std::shared_ptr<AudioBuffer> audioBuffer =
      std::make_shared<AudioBuffer>(960 * 2, 2);
  MicrophoneFactory microphoneFactory(loggerFactory);
  std::shared_ptr<Microphone> microphone =
      microphoneFactory.createMicrophone(audioBuffer, "hw:3,0");

  // starts service threads instantly, but will not run anything
  // TODO: Create pattern that creates services while adding them to the
  // sequencer, as this prevents dangling threads.
  auto serviceOne = std::make_unique<MicrophoneService>(
      "1", 10, maxPriority - 1, SERVICES_CORE, loggerFactory, audioBuffer,
      microphone, serviceConfig);
  auto serviceTwo =
      std::make_unique<FFTService>("2", 10, maxPriority - 2, SERVICES_CORE,
                                   loggerFactory, audioBuffer, serviceConfig);
  ...
  if (realTimeSettings->outputType() == CONSOLE) {
    ...
    auto serviceThree = std::make_unique<BeeperService>(
        "3", 100, maxPriority - 3, SERVICES_CORE, loggerFactory, audioBuffer,
        serviceConfig);
    ...
    auto serviceThree = std::make_unique<LEDBlinker>(
        "3", 200, maxPriority, SERVICES_CORE, loggerFactory, serviceConfig);
    sequencer->addService(std::move(serviceThree));
  } else {
    throw std::runtime_error("not yet supported - to be implemented");
  }

  auto serviceFour = std::make_unique<LogsToFileService>(
      "4", 200, minPriority, SERVICES_CORE, loggerFactory, serviceConfig);
